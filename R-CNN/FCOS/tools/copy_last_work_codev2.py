...
Запуск (пример):
  python R-CNN/FCOS/train_fcos.py \
    ...
    Запуск на мини датасетах (пример):
python R-CNN/FCOS/train_fcos.py \
...
# [PATCH] Runtime stability knobs (avoid IDE crashes & deadlocks)
...
# Мягкий импорт FCOS из AdelaiDet: если нет — не падаем, просто отметим None
try:
...
class FCOSDetector(nn.Module):  # noqa: N801
    ...
    def _ensure_targets(self, batched_inputs, image_sizes):
        """
        Гарантируем, что у каждого d есть d['instances'] (даже пустой),
        и корректно проставлены размеры изображения.
        """
        for i, d in enumerate(batched_inputs):
            if "instances" not in d or d["instances"] is None:
                h, w = image_sizes[i]
                inst = Instances((h, w))
                ...
                # выставим размеры, если не выставлены
                if not hasattr(d["instances"], "image_size") or d["instances"].image_size is None:
                    h, w = image_sizes[i]
    ...
    def forward(self, batched_inputs):
        ...
        # Гарантируем наличие targets (исправляет случаи 'targets is None')
        self._ensure_targets(batched_inputs, images.image_sizes)

        # # --- жёсткая проверка, что targets есть и не None ---
        # for i, d in enumerate(batched_inputs):
        #     if d.get("instances", None) is None:
        #         h, w = images.image_sizes[i]
        #         inst = Instances((h, w))
        #         inst.set("gt_boxes", Boxes(torch.zeros((0, 4), device=self.device)))
        #         inst.set("gt_classes", torch.zeros((0,), dtype=torch.int64, device=self.device))
        #         d["instances"] = inst
        # # опционально: быстрый санити-лог на первых шагах
        # if self.training and getattr(self, "_dbg_once", True):
        #     nz = [int(len(d["instances"])) for d in batched_inputs]
        #     print(f"[FCOSShim] gt per image (first step): {nz}")
        #     self._dbg_once = False
        #
        #
        # --- глубокая санация targets: гарантируем нужные поля и типы ---
        for i, d in enumerate(batched_inputs):
            h, w = images.image_sizes[i]
            inst = d.get("instances", None)

            # если инстансов нет — создаём пустые
            if inst is None:
                inst = Instances((h, w))
                d["instances"] = inst

            # выставим корректный размер изображения
            if not hasattr(inst, "image_size") or inst.image_size is None:
                inst._image_size = (h, w)

            # обязательные поля для FCOS
            if not inst.has("gt_boxes"):
                inst.set("gt_boxes", Boxes(torch.zeros((0, 4), device=self.device)))
            else:
                # клип на всякий случай (после ресайза)
        ...
        if self.training:
            ...
            for d in batched_inputs:
                inst = d["instances"]
                n_cls = int(inst.gt_classes.numel()) if inst.has("gt_classes") else 0
                n_box = int(inst.gt_boxes.tensor.shape[0]) if inst.has("gt_boxes") else 0
                total_gt_classes += n_cls
                total_gt_boxes += n_box

            if total_gt_classes == 0 and total_gt_boxes == 0:
                # --- нулевые лоссы с grad_fn ---
                dummy = None
                for p in self.model.parameters():
                    if p.requires_grad:
                        v = p.sum() * 0.0
                        dummy = v if dummy is None else (dummy + v)
                if dummy is None:  # на случай, если нет обучаемых параметров
        ...
        # ВАЖНО: первым именно batched_inputs, features — по имени!
        # ВАЖНО: первым именно batched_inputs, features — по имени!
        assert isinstance(features_dict, dict), "FPN must return a dict of feature maps"
        print("[DBG] fpn keys:", list(features_dict.keys())[:5], "in_features:",
              getattr(self.model, "in_features", None))

        try:
            # return self.model(features=features_dict, batched_inputs) #ошибка
            return self.model(batched_inputs, features=features_dict)  #ошибка [FCOSShim][WARN] AdelaiDet вернул targets=None (пустой батч). Возвращаю нулевые лоссы с grad_fn.
            # return self.model(features=features_dict, target=batched_inputs) #ошибка FCOS.forward() got an unexpected keyword argument 'target'
        except TypeError as e:
            if "NoneType" in str(e) and "has no len" in str(e):
                print("[FCOSShim][WARN] AdelaiDet вернул targets=None (пустой батч). "
                      "Возвращаю нулевые лоссы с grad_fn.")
                dummy = None
                for p in self.model.parameters():
                    if p.requires_grad:
                        v = p.sum() * 0.0
        ...
        # return self.model(features_dict, batched_inputs)
...
def mapper_with_augs(dataset_dict):
    ...
    # ВАЖНО: клипнем боксы в новые границы, иначе у библиотек могут быть сюрпризы
...
def main():
    ...
    # [PATCH] multiprocessing: «spawn» устойчивее для PyCharm/форков OpenCV/BLAS
